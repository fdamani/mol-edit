...
for i in range(0, n_best*num_evaluate, n_best):
	...
	for j in range(0, n_best):
		...
		try:
			...
			vals.append(properties.qed(sx))
		except:
			vals.append(-100.)
	ind = i+np.argmax(vals)
	qedvals.append(np.max(vals))
	top_qed_X.append(X.iloc[ind].values[0])
pd.DataFrame(top_qed_X).to_csv(output_file, header=None, index=None)
